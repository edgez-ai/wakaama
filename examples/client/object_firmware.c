...
static volatile bool s_fw_ota_in_progress = false;
...
static void ota_task(void *pvParameter)
{
    ...
    s_fw_ota_in_progress = true;
    s_fw_ota_cancel_requested = false;
    s_fw_active_data = data;
    ...
    while (1) {
        if (s_fw_ota_cancel_requested) {
            ESP_LOGW(FW_TAG, "OTA cancellation requested; aborting update");
            data->result = FW_RESULT_UPDATE_FAILED;
            data->state = FW_STATE_IDLE;
            esp_https_ota_abort(https_ota_handle);
            ota_log_heap("after_abort_cancel");
            goto ota_end;
        }
