...
typedef void (*gateway_device_update_callback_t)(unsigned int device_id, uint16_t new_instance_id);

// Callback function type for deleting device from external storage
typedef void (*gateway_device_delete_callback_t)(unsigned int device_id, uint16_t instance_id);
...
typedef void (*gateway_psk_write_callback_t)(unsigned int device_id, uint16_t instance_id, const char *identity, const uint8_t *psk, size_t psk_length, const char *server);
...
typedef struct _gateway_instance_
{
    ...
    unsigned int device_id;            // 32-bit device ID (read-only)
    ...
    unsigned int model;                // Device model (read-only)
...
uint8_t gateway_add_instance(lwm2m_object_t * objectP, uint16_t instanceId, unsigned int device_id, connection_type_t conn_type, unsigned int model, const char *mac_address);
...
int gateway_get_device_id(lwm2m_object_t * objectP, uint16_t instanceId, unsigned int *out);
...
int gateway_get_model(lwm2m_object_t * objectP, uint16_t instanceId, unsigned int *out);
...
uint8_t gateway_remove_and_recreate_instance(lwm2m_object_t * objectP, uint16_t old_instanceId, 
                                             unsigned int device_id, uint16_t new_instance_id, connection_type_t conn_type);
