                    ...
                    size_t length)
{
    ...
    in_port_t port = 0; // Initialize to avoid maybe-uninitialized warning
...
static int get_psk_info(struct dtls_context_t *ctx,
    ...
    if (cnx == NULL)
    {
    LOG("get_psk_info: session not found");
    ...
    switch (type) {
        case DTLS_PSK_IDENTITY:
        {
            ...
            {
                LOG("get_psk_info: buffer too small for psk_identity");
                return dtls_alert_fatal_create(DTLS_ALERT_INTERNAL_ERROR);
            }
        ...
        case DTLS_PSK_KEY:
        {
            ...
            {
                LOG("get_psk_info: buffer too small for psk");
                return dtls_alert_fatal_create(DTLS_ALERT_INTERNAL_ERROR);
            }
        ...
        default:
            LOG("get_psk_info: unsupported request type");
    }
...
int get_port(struct sockaddr *x)
{
   if (x->sa_family == AF_INET)
   {
    ...
    LOG("get_port: non IPV4 or IPV6 address");
       return  -1;
   }
}
...
int sockaddr_cmp(struct sockaddr *x, struct sockaddr *y)
{
    ...
    LOG("sockaddr_cmp: non IPV4 or IPV6 address");
        return 0;
    }
}
...
int create_socket(const char * portStr, int ai_family)
{
    ...
    // Create and bind the socket once, keep it open
...
int connection_handle_packet(dtls_connection_t *connP, uint8_t * buffer, size_t numBytes){
    ...
    if (connP->dtlsSession != NULL)
    {
       ...
       if (result !=0) {
           LOG("connection_handle_packet: error dtls handling message");
       }
        return result;
    } else {
...
int connection_rehandshake(dtls_connection_t *connP, bool sendCloseNotify) {
    ...
    if (result !=0) {
    LOG("connection_rehandshake: error dtls reconnection");
    }
    return result;
}
