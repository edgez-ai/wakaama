...
/*
 * ESP-IDF/newlib toolchains may provide a legacy POSIX DES function
 *   void encrypt(char block[64], int edflag);
 * whose prototype conflicts with TinyDTLS's internal static inline
 * helper named 'encrypt' below. To avoid a conflicting types error
 * when building for ESP32, we rename the local helper through a macro.
 */
#ifdef ESP_PLATFORM
#define encrypt tinydtls_ccm_local_encrypt
#endif
...
#ifdef ESP_PLATFORM
#undef encrypt
#define encrypt tinydtls_ccm_local_encrypt /* ensure subsequent references stay mapped */
#endif
